...
```
hierograph-mcp/io.hierograph.mcp.server/target/io.hierograph.mcp.server-0.1.0-SNAPSHOT-exec.jar
```

Run it with:

```bash
java -jar hierograph-mcp/io.hierograph.mcp.server/target/io.hierograph.mcp.server-0.1.0-SNAPSHOT-exec.jar
```

> The repackaged (self-contained) jar carries the `exec` classifier so the plain
> `io.hierograph.mcp.server-0.1.0-SNAPSHOT.jar` remains the module's main artifact —
...
Docker daemon) and uses Spring Boot's built-in image builder — [Cloud Native
Buildpacks](https://buildpacks.io), via `spring-boot:build-image`. There is **no Dockerfile**; the
buildpack assembles an optimized, layered OCI image directly from the repackaged jar.
...
> The first build pulls the Paketo builder/run images (`paketobuildpacks/builder-jammy-base`), which
> can take a few minutes; subsequent builds are cached and fast.
